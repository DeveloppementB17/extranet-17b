<?php

declare(strict_types=1);

namespace App\Command;

use App\Entreprise\LegacyTimeCreditExchange;
use Symfony\Component\Console\Attribute\AsCommand;
use Symfony\Component\Console\Command\Command;
use Symfony\Component\Console\Input\InputArgument;
use Symfony\Component\Console\Input\InputInterface;
use Symfony\Component\Console\Output\OutputInterface;
use Symfony\Component\Console\Style\SymfonyStyle;

#[AsCommand(
    name: 'app:build-legacy-time-credits-json',
    description: 'Génère un JSON d’import des crédits temps depuis un dump SQL de l’ancien extranet.',
)]
final class BuildLegacyTimeCreditsJsonCommand extends Command
{
    public function __construct(
        private readonly LegacyTimeCreditExchange $exchange,
    ) {
        parent::__construct();
    }

    protected function configure(): void
    {
        $this
            ->addArgument('dump', InputArgument::REQUIRED, 'Chemin vers le dump .sql ou .sql.gz')
            ->addArgument('output', InputArgument::OPTIONAL, 'Fichier JSON de sortie');
    }

    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        $io = new SymfonyStyle($input, $output);
        $dump = (string) $input->getArgument('dump');
        $outputPath = $input->getArgument('output');
        if (!\is_string($outputPath) || $outputPath === '') {
            $outputPath = dirname(__DIR__, 2).'/var/exports/legacy-time-credits.json';
        }

        try {
            $items = $this->exchange->itemsFromSqlDump($dump);
            $payload = $this->exchange->buildPayload($items, 'sql-dump:'.basename($dump));
            $json = $this->exchange->encodeJson($payload);

            $dir = dirname($outputPath);
            if (!is_dir($dir) && !mkdir($dir, 0775, true) && !is_dir($dir)) {
                throw new \RuntimeException(sprintf('Impossible de créer le dossier %s', $dir));
            }
            if (file_put_contents($outputPath, $json) === false) {
                throw new \RuntimeException(sprintf('Écriture impossible : %s', $outputPath));
            }
        } catch (\Throwable $e) {
            $io->error($e->getMessage());

            return Command::FAILURE;
        }

        $interventionCount = 0;
        foreach ($payload['credits'] as $credit) {
            $interventionCount += \count($credit['interventions']);
        }

        $io->success(sprintf(
            'Fichier généré : %s (%d crédit(s), %d intervention(s))',
            $outputPath,
            \count($payload['credits']),
            $interventionCount,
        ));

        return Command::SUCCESS;
    }
}
